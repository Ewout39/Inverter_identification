...
#Power flow analysis
function optimal_power_flow_analysis!(math, load_profiles, solar_profile, S_rated, PF, varP_curve, repitition)
    _PMD.add_start_vrvi!(math)
    for timestep in 1:10 #TODO change back to 35136 (or from April till september)
        insert_load_profiles!(math, load_profiles, timestep, solar_profile, PF, S_rated, varP_curve)
        res = MyVVPackage.solve_mc_opf(math, MyVVPackage.IVRENPowerModelVoltVar, optimizer_with_attributes(Ipopt.Optimizer, "max_iter" => 2000, "print_level" => 0), build_mc_opf = MyVVPackage.build_mc_opf)
...
for current_repitition in 1:repititions
    ...
    powerplot(math, width=1000, height = 1000, bus=(:size=>200))
    optimal_power_flow_analysis!(math, load_profiles, solar_profile, S_inverter, PF, varP_curve, current_repitition)
end
